...
namespace AtomicApps.HCUnavinarCore
{
    ...
    public class CanvasSafeArea : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (_adsService.CurrentBannerState.IsBannerActive)
            {
                UpdateSafeAreaWithAdsBanner(_adsService.CurrentBannerState);
            }
            
            _adsService.OnBannerStateChanged += UpdateSafeAreaWithAdsBanner;
        }
        ...
        private void UpdateSafeAreaWithAdsBanner(BannerStateData data)
        {
            ChangeSafeAreaForAdsBanner(data.IsBannerActive ? data.BannerHeight : -data.BannerHeight);
        }

        public void ChangeSafeAreaForAdsBanner(float bannerHeight)
        {
            RectTransform rectTransform = m_RectTransform;
            float currentBottom = rectTransform.offsetMin.y;
            rectTransform.offsetMin = new Vector2(rectTransform.offsetMax.x, currentBottom + bannerHeight);
        }
